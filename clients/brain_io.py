...
class BrainIO:
    ...
    async def append_file(self, relative_path: str, content: str) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: file_path.write_text(new_content, encoding='utf-8')
            )
    ...
    async def list_files(
        ...
        try:
            ...
            async def glob_files():
                ...
                if relative:
                    return [str(m.relative_to(self.brain_path)) for m in matches if m.is_file()]
                else:
                    return [str(m) for m in matches if m.is_file()]
            
            files = await loop.run_in_executor(
                None,
                lambda: glob_files() if asyncio.iscoroutinefunction(glob_files) else None
            )
            
            # Sync version if async didn't work
            matches = list(self.brain_path.glob(pattern))
            if relative:
                files = [str(m.relative_to(self.brain_path)) for m in matches if m.is_file()]
            else:
    ...
    async def ensure_folder(self, relative_path: str) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: folder_path.mkdir(parents=True, exist_ok=True)
            )
    ...
    async def get_recent_files(
        ...
        try:
            ...
            recent.sort(reverse=True, key=lambda x: asyncio.run(self.get_file_timestamp(x)) or 0)
